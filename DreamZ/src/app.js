...
setInterval(() => {
  document.title = alertShow ? 'E-com' : 'Dreamz';
...
class UI {
  ...
  getCartBtns() {
    //? the getCartBtns() function is responsible for selecting "Add to Cart" buttons, updating their state based 
    //?on the contents of the shopping cart, and handling the logic when a button is clicked to add a product to the cart. 
  ...
  setCartValue(cart) {
    //? This function essentially calculates the total price of the items in the shopping cart and updates the corresponding HTML elements to display this information on the webpage.
    //-> total price of cart
    let tempCartItems = 0;
    ...
    cartTotal.textContent = `Total price: $ ${totalPrice.toFixed(2)}`;//toFixed is for decimal values
  ...
  addCartItem(cartItem) {
    ...
    div.classList.add('cart-item'); //!Add css with the name cart-item
